...
- **codegraph** — multi-language code graph (Rust, Python, JS/TS, Go, Java, C/C++, C#, Kotlin, Scala, Ruby, PHP, Swift): index, defs, refs, callers, impls, path (BFS call-path between two fns), search (semantic symbol), stats. `limit` caps output. `project` = that workspace repo's own graph for indexing AND queries — per-repo graphs never clobber each other.
