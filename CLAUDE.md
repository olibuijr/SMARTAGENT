...
**Status (v0.1.0):** 20 crates done, tested, committed — semdb, httpc, memory, codegraph, codeindex, vault, skills, schedule, search, notify, secrets, browser, orchestrate, mcp, sandbox, context, evals, rag, voice, supervise. Each capability crate has a pi extension in `extensions/` (httpc is a shared library, akurai-router is the provider glue, session-memory is a hookless extension). `cargo test --workspace` green; `./build.sh` gate additionally lints extensions and smoke-tests that all 18 active crate tools register in pi (voice is built but delisted — no titan speech server); no file >1000 lines; zero crates.io deps. Remote: `github.com/olibuijr/SMARTAGENT` (private). Security P1s from the multi-agent review are fixed — see AGENTS.md "Security posture". Long-running services (scheduler daemon, headless chromium) are managed by the internal `supervise` process manager, not systemd — see `ops/README.md`. Post-v0.1.0: an adversarial-council pass expanded every tool with scope/limit/terse flags + new actions (browser 8 actions, memory update/forget/promote, search time-range/site, schedule --at, rag --url, codegraph impls/path, etc.) — see `CHANGELOG.md` (Unreleased) and the "token discipline" note in `AGENT_TOOLS.md`. **Project status SSOT: notes MCP note 39** ("SMARTAGENT — pure-Rust AI agent (dev status)").
