...
The public landing page at **smartagent.olibuijr.com** lives in `site/`: it is a playable pixel-world rendered with three.js WebGPU, deployed with `site/deploy.sh`, and includes a Game Boy-style mobile frame; treat this as documentation-only context unless a task explicitly asks for site implementation work.
...
**AkurAI-Notes is the priority location for durable documentation across all repos and projects.** Use the hosted AkurAI-Notes MCP server endpoint `https://akurai-notes.olibuijr.com/mcp` (configured as `mcp_akurai_notes_url` in `config/smartagent.conf`; service home `https://akurai-notes.olibuijr.com`) before writing long-lived docs to the local vault or ad-hoc markdown. Local repo docs remain the source for operational instructions that must ship with code, and local `vault` notes are fallback/mirror material when the AkurAI-Notes MCP server is unavailable; the canonical cross-repo/project note should live in AkurAI-Notes. Authentication must be policy-safe: use a stored MCP bearer token secret (expected secret name `akurai_notes_mcp_token`, corresponding to the remote `AKURAI_NOTES_MCP_TOKEN`) via `secrets get`, then call `mcp` with `auth_env`; never paste IDP passwords or MCP tokens into docs, memory, task comments, or shell logs. If the MCP token is missing, record the blocker and update local markdown only. EC2/VM operations use the `akurai-ec2` CLI and the `akurai-mail` SSH alias; do not read or print remote token files unless a credential-handling task explicitly permits a policy-safe path.
...
- **`semdb` is the shared storage library** other crates depend on via path dep. Its public API: `semdb::storage::Db` (`create`/`open`/`put`/`get`/`delete`/`index`), `semdb::cli::search(db, query, k, exact)` (cosine/HNSW), `semdb::http::fetch_embedding(host, port, model, text)`, `semdb::config::Config` (`load`/`resolve`/`workspaces_dir`/`data_dir`/`semdb_socket`), `semdb::json` (parse/escape/Value). **The `semdb` CLI is a thin client of the `semdb serve` daemon** (`semdb::{server,client,wire}`): the daemon opens each `.semdb` ONCE and holds it behind a per-db `RwLock` (concurrent reads, O(1) appends, background auto-compaction), served over `.pi/semdb.sock`. CLI verbs route through the client and **ERROR if the daemon is down — no direct-file fallback** (single owner; a process reaching around the daemon would clobber its writes). Registered as the first `supervise` service; start with `supervise up semdb`. In-process library callers (`Db::open`) are for per-crate dbs the daemon does not own. Never write ephemeral/UI state into the shared semantic db (that fs-watch→embed→write loop was the 2026-07-03 CPU-storm root cause). `httpc` is the shared HTTP/HTTPS+JSON library (`httpc::get`/`post_json`/`request`, `httpc::json`); HTTPS uses the system `openssl s_client` helper and `SMARTAGENT_HTTPC_CA_FILE` for private roots. Non-semantic tables store rows with a placeholder `[0.0]` vector.
