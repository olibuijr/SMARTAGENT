...
pub fn call(token: &str, method: &str, body: &str) -> Result<Value, String> {
    ...
    let out = Command::new("curl")
        .args(["-s", "--max-time", "20", "-X", "POST", &url, "-H", "Content-Type: application/json", "-d", body])
        .output()
        .map_err(|e| format!("run curl: {e}"))?;
    if !out.status.success() {
        return Err(format!("curl {method} exit {:?}: {}", out.status.code(), String::from_utf8_lossy(&out.stderr).trim()));
    }
    let v = json::parse(&String::from_utf8_lossy(&out.stdout)).map_err(|e| format!("bad {method} json: {e}"))?;
    if !matches!(v.get("ok"), Some(Value::Bool(true))) {
        return Err(format!("{method} not ok: {}", v.get("description").and_then(Value::as_str).unwrap_or("?")));
    }
...
pub fn get_updates(token: &str, offset: u64, timeout: u64) -> Result<Value, String> {
    ...
    let out = Command::new("curl")
        ...
        .output()
        .map_err(|e| format!("run curl: {e}"))?;
    let v = json::parse(&String::from_utf8_lossy(&out.stdout)).map_err(|e| format!("bad getUpdates json: {e}"))?;
    if !matches!(v.get("ok"), Some(Value::Bool(true))) {
        return Err(format!("getUpdates not ok: {}", v.get("description").and_then(Value::as_str).unwrap_or("?")));
    }
    Ok(v.get("result").cloned().unwrap_or(Value::Null))
}
...
pub fn send_message(token: &str, chat: &str, text: &str, markdown: bool) -> Result<i64, String> {
    let pm = if markdown { r#","parse_mode":"Markdown""# } else { "" };
    let body = format!(r#"{{"chat_id":"{}","text":"{}"{}}}"#, json::escape(chat), json::escape(text), pm);
    let r = call(token, "sendMessage", &body)?;
    r.get("message_id").and_then(Value::as_f64).map(|f| f as i64).ok_or_else(|| "sendMessage: no message_id".into())
}

/// editMessageText — used repeatedly for streaming (throttle at the caller).
pub fn edit_message(token: &str, chat: &str, message_id: i64, text: &str, markdown: bool) -> Result<(), String> {
    let pm = if markdown { r#","parse_mode":"Markdown""# } else { "" };
    let body = format!(r#"{{"chat_id":"{}","message_id":{},"text":"{}"{}}}"#, json::escape(chat), message_id, json::escape(text), pm);
