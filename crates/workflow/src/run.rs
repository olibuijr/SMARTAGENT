...
impl Store {
    ...
    pub fn all(&self) -> Result<Vec<Run>, String> {
        ...
        let mut rs: Vec<Run> = db
            ...
            .collect();
        rs.sort_by_key(|r| r.id.strip_prefix("W-").and_then(|s| s.parse::<u64>().ok()).unwrap_or(0));
        Ok(rs)
    }

    /// Most recent run still in `running` state.
    pub fn latest_running(&self) -> Result<Option<Run>, String> {
        Ok(self.all()?.into_iter().rev().find(|r| r.status == "running"))
    }
}

fn encode(r: &Run) -> String {
    let ev = r.evidence.iter().map(|e| format!("\"{}\"", json::escape(e))).collect::<Vec<_>>().join(",");
    format!(
...
fn decode(id: &str, meta: &str) -> Run {
    let v = match json::parse(meta) {
        Ok(v) => v,
        Err(_) => return Run { id: id.to_string(), ..Run::default() },
    };
...
mod tests {
    ...
    fn roundtrip_and_latest_running() {
        ...
        let r = Run { id: "W-1".into(), wf: "demo".into(), task: "T-9".into(), step: 1, status: "running".into(), started: 5, updated: 6, evidence: vec!["step one \"done\"".into()] };
        s.put(&r).unwrap();
