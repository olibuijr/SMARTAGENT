    ...
    match args.first().map(String::as_str) {
        Some("tools") => {
            ...
            if let Some(f) = flag(args, "--filter") {
                ...
                tools.retain(|(n, d)| {
                    n.to_ascii_lowercase().contains(&fl) || d.to_ascii_lowercase().contains(&fl)
                });
            }
            if tools.is_empty() {
                return Ok("no tools".into());
            }
...
const HELP: &str = r#"
mcp — Model Context Protocol client (stdio + HTTP/HTTPS)
